# Documentación SoftDomiFood

Esta carpeta contiene toda la documentación del proyecto organizada por categorías.

## 📁 Estructura de Documentación

### 🏗️ Architecture (`architecture/`)
Documentación de arquitectura, diseño y contexto del negocio:
- `BUSINESS_CONTEXT_IRIS.md` - Contexto de negocio y alcance del proyecto
- `REFINED_BACKLOG.md` - Backlog refinado del proyecto
- `user_stories.md` - Historias de usuario
- `AS-IS_Radiografia.txt` - Análisis del estado actual
- `SISTEMA_RESENAS_COMPLETO.md` - Sistema completo de reseñas
- `AI_WORKFLOW.md` - Workflow de integración con IA
- `DATABASE_README.md` - Documentación de la base de datos
- `ANALISIS_PATRONES_SOLID.md` - Análisis de patrones SOLID aplicados

### ⚙️ Setup (`setup/`)
Guías de instalación y configuración:
- `DESARROLLO-LOCAL.md` - Guía de desarrollo local
- `INICIAR_EN_PODMAN.md` - Guía para iniciar con Podman
- `MIGRATION_GUIDE.md` - Guía de migración

### 🐛 Fixes (`fixes/`)
Documentación de correcciones y bugs resueltos:
- `FAVORITES_FIX.md` - Corrección del sistema de favoritos
- `FAVORITES_FIXED_SUMMARY.md` - Resumen de corrección de favoritos
- `FAVORITES_SYNC_FIX.md` - Corrección de sincronización de favoritos
- `SUMMARY_FAVORITES_FIX.md` - Resumen completo de correcciones

### 🧪 Testing (`testing/`)
Documentación de pruebas, auditorías y QA:
- `TEST_PLAN.md` - Plan de pruebas
- `TEST_CASES.md` - Casos de prueba
- `AUDIT_REPORT_GENERAL.md` - Reporte de auditoría general
- `AUDIT_REPORT_API.md` - Reporte de auditoría de la API
- `AUDIT_REPORT_WORKER.md` - Reporte de auditoría del worker
- `RESULTADOS_TESTS_REVIEWS.md` - Resultados de pruebas de reseñas
- `README_TESTS_REVIEWS.md` - Guía de pruebas de reseñas
- `CORRECCION_B104.md` - Corrección de issue B104
- `EXPLICACION_TESTS_PENDIENTES.md` - Explicación de tests pendientes
- `README_TESTING.md` - Guía general de testing
- `RESULTADOS_ANALISIS_SEGURIDAD.md` - Resultados de análisis de seguridad
- `RESULTADOS_EJECUCION.md` - Resultados de ejecución de pruebas
- `SOLUCION_3_IMPLEMENTADA.md` - Documentación de solución implementada
- `VER_RESULTADOS.md` - Guía para ver resultados de pruebas

## 📚 Navegación Rápida

- **¿Nuevo en el proyecto?** → Comienza con [`setup/DESARROLLO-LOCAL.md`](setup/DESARROLLO-LOCAL.md)
- **¿Necesitas contexto de negocio?** → Lee [`architecture/BUSINESS_CONTEXT_IRIS.md`](architecture/BUSINESS_CONTEXT_IRIS.md)
- **¿Vas a ejecutar pruebas?** → Consulta [`testing/TEST_PLAN.md`](testing/TEST_PLAN.md)
- **¿Problemas conocidos?** → Revisa la carpeta [`fixes/`](fixes/)

## 🔄 Mantenimiento de Documentación

Esta estructura de documentación sigue las mejores prácticas de organización de proyectos:

1. **Documentación centralizada**: Todos los archivos .md en una ubicación
2. **Categorización lógica**: Agrupación por propósito
3. **Nombres descriptivos**: Identificación clara del contenido
4. **Fácil navegación**: Índice y enlaces rápidos

Para contribuir con nueva documentación, coloca los archivos en la carpeta correspondiente según su propósito.
